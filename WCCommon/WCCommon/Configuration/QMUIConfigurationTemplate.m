...
#pragma mark - NavigationBar
    ...
    QMUICMI.navBarBackgroundImage = [[UIImage qmui_imageWithColor:UIColor.qd_tintColor] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 1, 0, 1) resizingMode:UIImageResizingModeStretch];   // NavBarBackgroundImage : UINavigationBar 的背景图，注意 navigationBar 的高度会受多个因素（是否全面屏、是否使用了 navigationItem.prompt、是否将 UISearchBar 作为 titleView）的影响，要检查各种情况是否都显示正常。
...
#pragma mark - Plain TableView
    ...
    QMUICMI.tableViewCellDisclosureIndicatorImage = [UIImage qmui_imageWithThemeProvider:^UIImage * _Nonnull(__kindof QMUIThemeManager * _Nonnull manager, NSString * _Nullable identifier, NSObject<QDThemeProtocol> * _Nullable theme) {
        return [identifier isEqualToString:WCThemeIdentifierDark] ? disclosureIndicatorImageDark : disclosureIndicatorImage;
